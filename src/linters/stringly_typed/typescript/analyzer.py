...
Overview: Provides TypeScriptStringlyTypedAnalyzer class that coordinates detection of
    ...
    TypeScriptCallTracker to find function calls with string arguments. Returns
    FunctionCallResult objects compatible with the Python analyzer format for unified
    cross-file analysis. Handles tree-sitter parsing gracefully and provides a single
    entry point for TypeScript analysis. Supports configuration options for filtering
    and thresholds.

Dependencies: TypeScriptCallTracker, StringlyTypedConfig, pathlib.Path
...
from ..python.analyzer import FunctionCallResult
...
class TypeScriptStringlyTypedAnalyzer:
    ...
    with string arguments ('process("active")', 'obj.setStatus("pending")').
